...
func Channellist(db *gorm.DB,ctx context.Context,limit,offset int)(model.ChannelDetails,error){
	...
	if token == SpecialToken{
	   ...
	   Where("tbl_channels.is_deleted = 0 and tbl_channels.is_active = 1 and tbl_channel_entries.status = 1").Count(&count).Error
	...
	if token == SpecialToken{
		...
		if channelEntryID!=nil || (channelID!=nil && channelEntryID!=nil) && limit==nil && offset==nil {
			...
			if entryerr==nil{
				...
				if modified_path!="" && os.Getenv("DOMAIN_URL")!=""{

					channelEntry.CoverImage = os.Getenv("DOMAIN_URL")+ modified_path

				}else{

					channelEntry.CoverImage = os.Getenv("LOCAL_URL")+ modified_path
				}
			...
			if entrieserr==nil && counterr==nil{
				...
				for _,entry := range channelEntries{
					...
					if modified_path!="" && os.Getenv("DOMAIN_URL")!=""{
						...
						entry.CoverImage = os.Getenv("LOCAL_URL")+ modified_path
					}
